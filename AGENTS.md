...
- `src/helpers/core` contains selective folder-level `index.ts` facades that import and re-export `@code-rag/core@0.1.10`; do not copy or reimplement CodeRAG internals.
- Application code imports CodeRAG capabilities through `@/helpers/core/index.js`.
- Production pipeline helpers under `src/helpers` (`file-scanner`, `source-processor`, `documentation-processor`, `chunk-enrichment`, `chunk-indexer`, `hybrid-retriever`, `context-builder`, `answer-generator`, `provider-factory`) wrap `@/helpers/core/index.js` and `@/config/index.js` only; services must go through these helpers instead of importing `@/helpers/core/index.js` directly once an equivalent helper exists.
- **Extended Language Support**: `src/helpers/extended-language-registry.ts` and `src/helpers/extended-tree-sitter-parser.ts` extend CodeRAG's base 12-language support to all 37 languages available in `tree-sitter-wasms` (including Kotlin, YAML, Bash, JSON, TOML, CSS, HTML, etc.) without forking the `@code-rag/core` package. These wrappers properly extend CodeRAG's `LanguageRegistry` and `TreeSitterParser` classes through the facade (`@/helpers/core/parser/index.js`) rather than duplicating CodeRAG's internal mappings. The `source-processor.ts` helper uses `ExtendedTreeSitterParser` instead of CodeRAG's base `TreeSitterParser` for full language coverage.
- External-client implementations (e.g. `OpenAICompatibleLlm`, an `LLMProvider` over HTTP) live under `src/connectors`, not `src/helpers`, per "Connectors construct and expose external clients." They are constructed only through `provider-factory`.
- **No hardcoded configuration in helpers or services.** Any value already defined in `config/app.yaml` (e.g. file scan exclude patterns, token budgets, retrieval weights, model settings) must be read through its `@/config` getter (`getIngestionConfig`, `getRetrievalConfig`, etc.) by the calling service and passed into helpers as explicit parameters — helpers must not read `@/config` for anything except prompt rendering (`renderPrompt`) and `provider-factory`'s provider construction. Prefer reusing an existing config key over inventing a new one; do not add new config surface (e.g. a parallel "include" list) to work around a hardcoded value when the existing key can be generalized instead — `file-scanner.ts`'s `excludePatterns` is deliberately the single source of file-scan filtering, with each pattern treated as a regular expression (`new RegExp(pattern)`, tested against the full file path) rather than a separate include/exclude pair; `config/app.yaml`'s default patterns are authored directly in regex syntax accordingly. If a genuinely new tunable is needed, add it to the relevant Zod schema in `src/types/config/*.schema.ts`, `config/app.yaml`, and the corresponding getter in `src/config/app-config.ts` first; only the ultimate fallback constant passed to `getConfigValue` may be a literal. Add/extend a `src/config/test/config-exposure.test.ts` case so every `app.yaml`/`prompts.yaml` key stays provably reachable through a getter.
